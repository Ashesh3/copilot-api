...
function loadLlmDebugLogs() { apiFetch('GET', '/dashboard/api/llm-debug').then(function(r) { if (r.ok) return r.json() }).then(function(d) { if (d) { llmDebugData = d; renderLlmDebugLogs() } }).catch(function() { showToast('Failed to load LLM debug logs', 'error') }); refreshTimers.llmDebug = setInterval(function() { if (currentSection !== 'llm-debug') return; apiFetch('GET', '/dashboard/api/llm-debug').then(function(r) { if (r.ok) return r.json() }).then(function(d) { if (d) { llmDebugData = d; renderLlmDebugLogs() } }).catch(function(){}) }, 5000) }
...
function renderLlmDebugLogs() { var c = document.getElementById('llm-debug-content'), n = document.getElementById('llm-debug-count'), qEl = document.getElementById('llm-debug-filter'), sEl = document.getElementById('llm-debug-status'), pEl = document.getElementById('llm-debug-path'); var entries = llmDebugData.entries || [], q = qEl ? qEl.value.trim().toLowerCase() : '', s = sEl ? sEl.value : 'all', p = pEl ? pEl.value : 'all', filtered = entries.filter(function(e) { return llmEntryMatches(e, q, s, p) }); if (n) n.textContent = filtered.length + ' of ' + (llmDebugData.count || 0) + ' calls'; if (!entries.length) { c.innerHTML = '<div class="empty-state"><p>No LLM calls captured in the last 10 minutes.</p></div>'; return } if (!filtered.length) { c.innerHTML = '<div class="empty-state"><p>No debug logs match this filter.</p></div>'; return } var html = '<div class="llm-debug-list">'; filtered.forEach(function(e) { html += renderLlmDebugRow(e) }); c.innerHTML = html + '</div>' }
...
function toggleLlmDebugDetail(id) { expandedLlmDebug[id] = !expandedLlmDebug[id]; renderLlmDebugLogs(); if (expandedLlmDebug[id] && !llmDebugDetails[id]) loadLlmDebugDetail(id) }
function loadLlmDebugDetail(id) { apiFetch('GET', '/dashboard/api/llm-debug/' + encodeURIComponent(id)).then(function(r) { if (r.ok) return r.json(); throw new Error('not found') }).then(function(d) { llmDebugDetails[id] = d; var el = document.getElementById('llm-detail-' + id); if (el) el.innerHTML = renderLlmDebugDetail(id) }).catch(function() { showToast('Failed to load debug log', 'error') }) }
...
function llmPanelText(button) { var panel = button.closest('.debug-panel'); if (!panel) return ''; var pre = panel.querySelector('pre'); if (pre) return pre.textContent || ''; var rows = []; panel.querySelectorAll('.debug-kv-key').forEach(function(k) { var v = k.nextElementSibling; rows.push((k.textContent || '') + ': ' + (v ? v.textContent || '' : '')) }); return rows.join('\n') }
