...
This is a historical review record. Current local SQLite deployment and
database download/recovery procedures are in the [storage runbook](../../sqlite-storage.md).
...
Recorded validation for that release: **3,954 passed, 0 failed, 26 explicit opt-in skips across 247 fresh-process files.** The PR records hosted CI results and the final merge revision. Those figures describe the old release and do not establish validation of the current checkout. External model responses in protocol tests were synthetic; live subscription inference is not inferred from those checks.
...
- Current persistence uses local SQLite through Bun's built-in driver.
